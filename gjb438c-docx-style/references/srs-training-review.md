...
| `derived` | 有明确合同父条文和派生依据；`source_ids`指向`kind=contract`的正式父来源，`basis`说明必要细化过程，并保留正式来源或`derivation`记录。方案可佐证，不能替代合同父来源；合同原件待确认仍须保留基线缺口 |
...
培训配置第5章只使用独立`kind=contract`来源原文台账与`contract_trace`建立正式合同正、反向表。未分配或尚未映射的合同来源仍列出处置，不能从现有需求反推合同义务全集。每项正式需求均有反向处置；没有合同父来源的pending条款明确为范围未确认，不给它补造合同终点。条文映射状态与文件确认状态分别展示。

方案等非合同来源台账与原`forwardTrace/reverseTrace`完整移至第6章“历史来源与佐证追踪”，不删除有效上层来源、派生依据或版本沿革。正式需求细表仍保留完整来源关系。显式`gjb-standard`保持原有第5章一般来源双向追踪行为。保存后审计同时核验合同正式表和历史关系，不能只把标题改成“合同”而保留方案终点。
